...
if(isset($_POST['form1'])){
    try {
        ...
        if (empty($_POST['name'])) {
            throw new Exception("hospital can not be empty.");
        }
       ...
       $statement = $db->prepare("UPDATE hospital  SET name=?,place=?,description=? WHERE id=?");
...
$statement = $db->prepare("SELECT * FROM hospital  WHERE id=?");
