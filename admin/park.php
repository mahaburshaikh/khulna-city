...
if (isset($_POST['form1'])) {
    ...
    try {
        ...
        $statement = $db->prepare("SHOW TABLE STATUS LIKE 'park'");
        $statement->execute();
        $result = $statement->fetchAll();
        foreach ($result as $row)
            $new_id = $row[10];

        $up_filename = $_FILES["park_image"]["name"];
        ...
        move_uploaded_file($_FILES["park_image"]["tmp_name"], "../park_image/" . $f1);

        $statement = $db->prepare("INSERT INTO park(name,description,place_image,place) VALUES(?,?, ?, ?)");
...
<form action="" method="post" enctype="multipart/form-data">
    <table class="tbl1">
        ...
        <tr><td>Image</td></tr>
        <tr><td><input type="file" name="park_image"></td></tr>
