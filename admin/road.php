...
if (isset($_POST['form1'])) {
    ...
    try {
        ...
        $statement = $db->prepare("SHOW TABLE STATUS LIKE 'road'");
        $statement->execute();
        $result = $statement->fetchAll();
        foreach ($result as $row)
            $new_id = $row[10];

        $up_filename = $_FILES["road_image"]["name"];
        ...
        move_uploaded_file($_FILES["road_image"]["tmp_name"], "../road_image/" . $f1);

        $statement = $db->prepare("INSERT INTO road(name,description,place_image,place) VALUES(?,?, ?, ?)");
...
<form action="" method="post" enctype="multipart/form-data">
    <table class="tbl1">
        ...
        <tr><td>Image</td></tr>
        <tr><td><input type="file" name="road_image"></td></tr>
