...
if(isset($_POST['form1'])){
    try {
        ...
        if (empty($_POST['name'])) {
            throw new Exception("park can not be empty.");
        }
       ...
       $statement = $db->prepare("UPDATE park SET name=?,place=?,description=? WHERE id=?");
        ...
        $success_message = " park details updated successfully.";
    } catch (Exception $e) {
...
$statement = $db->prepare("SELECT * FROM park WHERE id=?");
$statement->execute(array($id));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
    ?>
        
        <tr><td>park Name</td></tr>
