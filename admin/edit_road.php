...
if(isset($_POST['form1'])){
    try {
        ...
        if (empty($_POST['name'])) {
            throw new Exception("road can not be empty.");
        }
       ...
       $statement = $db->prepare("UPDATE park SET name=?,place=?,description=? WHERE id=?");
        $statement->execute(array($_POST['name'],$_POST['place'],$_POST['description'],$id));

        $success_message = " road details updated successfully.";
    } catch (Exception $e) {
...
$statement = $db->prepare("SELECT * FROM road  WHERE id=?");
$statement->execute(array($id));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
    ?>
        
        <tr><td>road  Name</td></tr>
