...
if(isset($_POST['form1'])){
    try {
        ...
        if (empty($_POST['name'])) {
            throw new Exception("market can not be empty.");
        }
       ...
       $statement = $db->prepare("UPDATE market  SET name=?,place=?,description=? WHERE id=?");
        $statement->execute(array($_POST['name'],$_POST['place'],$_POST['description'],$id));

        $success_message = " market  details updated successfully.";
    } catch (Exception $e) {
...
$statement = $db->prepare("SELECT * FROM market  WHERE id=?");
$statement->execute(array($id));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
    ?>
        
        <tr><td>market  Name</td></tr>
