...
if (isset($_POST['form1'])) {
    ...
    try {
        ...
        $statement = $db->prepare("SHOW TABLE STATUS LIKE 'beautiful_place'");
        $statement->execute();
        $result = $statement->fetchAll();
        foreach ($result as $row)
            $new_id = $row[10];

        $up_filename = $_FILES["education_image"]["name"];
        ...
        move_uploaded_file($_FILES["education_image"]["tmp_name"], "../education_image/" . $f1);

        $statement = $db->prepare("INSERT INTO education(name,description,place_image,place) VALUES(?,?, ?, ?)");
        $statement->execute(array($_POST['name'], $_POST['description'], $f1,$_POST['place']));

        $success_message = "Place details inserted successfully.";
    } catch (Exception $e) {
...
<form action="" method="post" enctype="multipart/form-data">
    <table class="tbl1">
        ...
        <tr><td>Image</td></tr>
        <tr><td><input type="file" name="education_image"></td></tr>
