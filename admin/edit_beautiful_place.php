...
if(isset($_POST['form1'])){
    try {
        ...
        if (empty($_POST['name'])) {
            throw new Exception(" beautiful_place can not be empty.");
        }
       ...
       $statement = $db->prepare("UPDATE beautiful_place SET name=?,place=?,description=? WHERE id=?");
        $statement->execute(array($_POST['name'],$_POST['place'],$_POST['description'],$id));

        $success_message = " beautiful_place details updated successfully.";
    } catch (Exception $e) {
...
$statement = $db->prepare("SELECT * FROM  beautiful_place WHERE id=?");
$statement->execute(array($id));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
    ?>
        
        <tr><td> beautiful_place Name</td></tr>
